            ...
            <div class="col-sm-12 col-md-4">
                ...
                if(!(strpos($tipoArchivo,"gif") || strpos($tipoArchivo,"jpg") || strpos($tipoArchivo,"jpeg") || strpos($tipoArchivo,"png")) && $tamanoArchivo>5000000){
                    ...
                    if(move_uploaded_file($_FILES['imagen']['tmp_name'], $nombreArchivo)){
                        ...
                        // Prepara consulta *
                        $sql = "UPDATE `noticias` SET `texto`='$textoN', `imagen`='$imagenBase64', `titulo`='$tituloN',`categoria`='$catN' WHERE id='$nId';";
                        // Ejecuta la consulta 
                        $result = $conn->query($sql);
                        if($result === TRUE){
                            echo "<h1>Noticia actualizada exitosamente!</h1";
                        }else{
                            echo "<h1>No se pudo actualizar la noticia, intentalo de nuevo</h1";
                        }
                        $conn->close();
                    }else{
                        $conn = new mysqli($servername, $username, $password, $dbname) or die("<h1>no pudo conectarse con la base de datos</h1");
                        // Prepara consulta *
                        $sql = "UPDATE `noticias` SET `texto`='$textoN', `titulo`='$tituloN',`categoria`='$catN' WHERE id='$nId';";
                        // Ejecuta la consulta 
                        $result = $conn->query($sql);
                        if($result === TRUE){
                            echo "<h1>Noticia actualizada exitosamente!</h1";
